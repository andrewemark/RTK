...
#ifndef rtkSuperShortScanBackProjectionImageFilter_hxx
#define rtkSuperShortScanBackProjectionImageFilter_hxx

#include "math.h"

#include "rtkSuperShortScanBackProjectionImageFilter.h"
...
void
SuperShortScanBackProjectionImageFilter<TInputImage, TOutputImage>::GenerateOutputInformation()
{
  // Check if detector is cylindrical
  if (this->m_Geometry->GetRadiusCylindricalDetector() != 0)
  {
    itkGenericExceptionMacro(
      << "Voxel-based SuperShortScan back projector can currently not handle cylindrical detectors");
  }
...
template <class TInputImage, class TOutputImage>
void
SuperShortScanBackProjectionImageFilter<TInputImage, TOutputImage>::DynamicThreadedGenerateData(
  const OutputImageRegionType & outputRegionForThread)
{
  ...
  for (unsigned int iProj = iFirstProj; iProj < iFirstProj + nProj; iProj++)
  {
    ...
    while (!itOut.IsAtEnd())
    {
      ...
      {
        itOut.Set(itOut.Get() + perspFactor_local * interpolator->EvaluateAtContinuousIndex(pointProj));
      }
...
template <class TInputImage, class TOutputImage>
void
SuperShortScanBackProjectionImageFilter<TInputImage, TOutputImage>::OptimizedBackprojectionX(
...
template <class TInputImage, class TOutputImage>
void
SuperShortScanBackProjectionImageFilter<TInputImage, TOutputImage>::OptimizedBackprojectionY(
  const OutputImageRegionType & region,
  const ProjectionMatrixType &  matrix,
  const ProjectionImagePointer  projection)
{
